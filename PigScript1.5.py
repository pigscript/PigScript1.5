...
class paint:	
	...
	def plotcurve(self):
		...
		try:
			col_k = float(color_period[0])
		except:
			col_k = 1
		try:
			col_b = float(color_period[1])
		except:
			col_b = 0
		try:
			col_c = float(color_period[2])
		except:
			col_c = 0
		...
		for n in range(num):
			ratio = (n+1)/num
			col_ratio = 1-abs((col_k*ratio+col_b)%2-1)
			if color_plan=='1':
				self.setcolor(col_b+col_k*n/num+col_c*n*n/(num*num),0,0,rainbow=True)
			elif color_plan=='2':
			...
			if curve_type=='0':
				self.polar(lambda x: 50*(x-2*math.pi*ratio)/lpn, start=2*math.pi*ratio, loop=lpn, dir=1)
			elif curve_type=='1':
				self.polar(lambda x: 50*(x-2*math.pi*ratio)/lpn, start=2*math.pi*ratio, loop=lpn, dir=-1)
			elif curve_type=='2':
				self.polar(lambda x: 300*(ratio+0.01), cx=a, cy=b)
			elif curve_type=='3':
				R = 280
				r = 0.6*R*ratio
				theta = math.asin(math.sqrt((R*R-r*r)/(R*R+r*r*ratio*ratio)))
				self.paraplot(lambda x: r/math.cos(x), lambda y: ratio*r*math.tan(y), start=-theta,end=theta)
				self.paraplot(lambda x: ratio*r*math.tan(x), lambda y: r/math.cos(y), start=-theta,end=theta)
				self.paraplot(lambda x: -r/math.cos(x), lambda y: ratio*r*math.tan(y), start=-theta,end=theta)
				self.paraplot(lambda x: ratio*r*math.tan(x), lambda y: -r/math.cos(y), start=-theta,end=theta)
			elif curve_type=='4':
				R = 280
				r = R*(1-ratio+1/num)**0.9
				theta = math.asin(math.sqrt((R*R-r*r)/(R*R+r*r*ratio*ratio)))
				self.paraplot(lambda x: r/math.cos(x), lambda y: ratio*r*math.tan(y), start=-theta,end=theta)
				self.paraplot(lambda x: ratio*r*math.tan(x), lambda y: r/math.cos(y), start=-theta,end=theta)
				self.paraplot(lambda x: -r/math.cos(x), lambda y: ratio*r*math.tan(y), start=-theta,end=theta)
				...
				self.paraplot(lambda x: 100*(ratio+1)*(math.cos(x)+math.cos(p*x)/p)/(1+1/p), 
				lambda y: 100*(ratio+1)*(math.sin(y)-math.sin(p*y)/p)/(1+1/p), start=0, end=lpn*2*math.pi)
		self.mouse.move(a,b)
	...
	def polar(self,f,start=0,loop=1,speed=1,cx=0,cy=0,dir=1):
		...
		while theta<=end:
			...
			theta += (i-1)*dtheta
			time.sleep(0.02)
		x=cx+round(f(theta)*math.cos(theta))
		y=cy-dir*round(f(theta)*math.sin(theta))
		self.mouse.drag(x,y)
		time.sleep(0.03)
